...
-- 添加恶魔饥饿值组件
AddComponentPostInit("demonhunger", function(self, inst)
    if inst:HasTag("vox") then
        self.demonhunger = 0
        self.max_demonhunger = 150
        
        function self:GetDemonPercent()
            return self.demonhunger / self.max_demonhunger
        end
        
        function self:SetDemonPercent(percent)
            self:SetDemonHunger(percent * self.max_demonhunger)
        end
        ...
        function self:DoDelta(delta)
            ...
            -- 当恶魔饥饿值为0时，普通食物只能回复5%饱食度
            if delta > 0 and self.demonhunger <= 0 then
                delta = delta * 0.05
            end
            ...
            if old ~= self.current then
                self.inst:PushEvent("hungerdelta", { oldpercent = old / self.max, newpercent = self.current / self.max, overtime = false })
            end
        ...
        function self:DoDemonDelta(delta)
            local old = self.demonhunger
            self.demonhunger = math.clamp(self.demonhunger + delta, 0, self.max_demonhunger)
            
            if old ~= self.demonhunger then
                self.inst:PushEvent("demonhungerdelta", { oldpercent = old / self.max_demonhunger, newpercent = self.demonhunger / self.max_demonhunger, overtime = false })
                
                -- 如果满了，进入暴走模式
                if self.demonhunger >= self.max_demonhunger and old < self.max_demonhunger then
                    self.inst:PushEvent("enterdemonrage")
                -- 如果清空了，退出暴走模式
                elseif self.demonhunger <= 0 and old > 0 and self.inst:HasTag("demonrage") then
                    self.inst:PushEvent("exitdemonrage")
                end
            end
            
            return self.demonhunger - old
        end

        function self:SetDemonHunger(amount)
            self:DoDemonDelta(amount - self.demonhunger)
        end
        
        function self:Pause()
            self.burning = false
            self.demonhungerburning = false
        end
        
        function self:Resume()
            self.burning = true
            self.demonhungerburning = true
        end
        ...
        function self:LongUpdate(dt)
            ...
            -- 恶魔饥饿值下降速度是普通饥饿值的75%
            if self.demonhungerburning then
                self:DoDemonDelta(-dt * self.hungerrate * 0.75)
            end
        end
...
-- 检测猴子位置的按键绑定
    local player = ThePlayer
        if player and player.prefab == "vox" then
            ...
            if #monkeys > 0 then
                -- 寻找最近的猴子逻辑...
                player.components.talker:Say("我感觉到附近有猴子!")
            else
