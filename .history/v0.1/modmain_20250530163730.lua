...
local function SetPlaceholderImages()
    -- 使用wilson的头像作为占位
    local wilson_images = {
        "images/saveslot_portraits/wilson.tex",
        "images/selectscreen_portraits/wilson.tex", 
        "images/selectscreen_portraits/wilson_silho.tex",
        "bigportraits/wilson.tex",
        "images/avatars/avatar_wilson.tex",
        "images/avatars/avatar_ghost_wilson.tex",
        "images/avatars/self_inspect_wilson.tex",
    }
    
    -- 复制为vox对应文件
    for _, path in ipairs(wilson_images) do
        local vox_path = path:gsub("wilson", "vox")
        -- 这里需要手动复制文件
    end
end


-- 在modmain.lua顶部添加配置选项
configuration_options = {
    {
        name = "vox_language_set",
        label = "语言设置",
        options = {
    ...
    {
        name = "demon_energy_rate",
        label = "恶魔能量消耗速度",
        options = {
            { description = "慢", data = 0.5 },
            { description = "正常", data = 0.75 },
            { description = "快", data = 1.0 },
        },
        default = 0.75,
    },
    {
        name = "monkey_sense_key",
        label = "猴子感应按键",
        options = {
...
local BOSS_ABILITIES = {
    ...
    spiderqueen = {
        name = "蜘蛛女王之力",
        description = "获得蜘蛛人的所有能力",
        duration = 5 * 480, -- 5天
        icon = "spider_queen_buff",
        effects = {
            tags = { "spiderwhisperer", "webwalker" },
            immunities = { "spider_poison" },
            bonuses = {
                web_speed_mult = 1.25,
                monster_meat_safe = true,
                spider_hiring = true,
                spider_nest_sleep = true,
            },
            -- 持续效果函数
            on_apply = function(inst)
                ...
                -- 在蛛网上行走速度加成
                inst.components.locomotor:SetExternalSpeedMultiplier(inst, "spider_web", 1.25)
            end,
            on_remove = function(inst)
                inst:RemoveTag("spiderwhisperer")
                inst.components.locomotor:RemoveExternalSpeedMultiplier(inst, "spider_web")
            end,
        }
    },
    ...
    dragonfly = {
        ...
        icon = "dragonfly_buff",
        effects = {
            tags = { "fire_immune", "heat_immune" },
            immunities = { "fire", "overheat" },
            bonuses = {
                fire_damage_mult = 0,
                summer_comfort = true,
            },
            on_apply = function(inst)
                inst.components.health:SetFireDamageScale(0, "dragonfly_buff")
                if inst.components.temperature then
                    inst.components.temperature:SetTemp(25) -- 保持适宜温度
                end
            end,
            on_remove = function(inst)
                inst.components.health:SetFireDamageScale(1, "dragonfly_buff")
            end,
        }
    },
    ...
    deerclops = {
        ...
        icon = "deerclops_buff",
        effects = {
            tags = { "freeze_immune", "cold_immune" },
            immunities = { "freeze", "cold" },
            bonuses = {
                winter_comfort = true,
                freeze_resistance = 999,
            },
            on_apply = function(inst)
                ...
                if inst.components.temperature then
                    inst.old_min_temp = inst.components.temperature.mintemp
                    inst.components.temperature.mintemp = -999
                end
            end,
            on_remove = function(inst)
                if inst.components.freezable then
                ...
                if inst.components.temperature and inst.old_min_temp then
                    inst.components.temperature.mintemp = inst.old_min_temp
                    inst.old_min_temp = nil
                end
            end,
        }
    },
    ...
    moose = {
        ...
        icon = "moose_buff",
        effects = {
            tags = { "lightning_immune", "rain_immune" },
            immunities = { "lightning", "wetness" },
            bonuses = {
                lightning_rod = true,
            ...
            on_apply = function(inst)
                if inst.components.moisture then
                    inst.old_moisture_rate = inst.components.moisture.rate
            ...
            on_remove = function(inst)
                if inst.components.moisture and inst.old_moisture_rate then
                    inst.components.moisture.rate = inst.old_moisture_rate
                    inst.old_moisture_rate = nil
                end
                if inst.components.playerlightningtarget then
                    inst.components.playerlightningtarget:SetHitChance(TUNING.LIGHTNING_CHANCE)
                end
            end,
        }
    },
    ...
    antlion = {
        ...
        effects = {
            ...
            on_apply = function(inst)
                -- 允许灵魂跳跃
                inst.can_soul_hop = true
                -- 添加跳跃技能
                if inst.components.playeractionpicker then
                    inst.soul_hop_action = ActionHandler(ACTIONS.BLINK, "soul_hop")
                end
            end,
            on_remove = function(inst)
                inst.can_soul_hop = false
                inst.soul_hop_action = nil
            end,
        }
    },
    ...
    bearger = {
        name = "熊獾之力",
        description = "砍树挖矿效率大幅提升",
        duration = 5 * 480,
        icon = "bearger_buff",
        effects = {
            tags = { "super_worker" },
            bonuses = {
                work_efficiency = 3.0, -- 效率提升200%
                ground_pound = true,
            },
            on_apply = function(inst)
                if inst.components.worker then
                    inst.components.worker:SetAction(ACTIONS.CHOP, 3.0)
                    inst.components.worker:SetAction(ACTIONS.MINE, 3.0)
                end
                -- 添加地面重击能力
                inst.ground_pound_action = ActionHandler(ACTIONS.TERRAFORM, "ground_pound")
            end,
            on_remove = function(inst)
                if inst.components.worker then
                    inst.components.worker:SetAction(ACTIONS.CHOP, 1.0)
                    inst.components.worker:SetAction(ACTIONS.MINE, 1.0)
                end
                inst.ground_pound_action = nil
            end,
        }
    },
    ...
    beequeen = {
        name = "蜂后之力",
        description = "霸体效果，被打不会僵直",
        duration = 5 * 480,
        icon = "bee_queen_buff",
        effects = {
            tags = { "super_armor", "no_stun" },
            bonuses = {
                stun_immunity = true,
            ...
            on_apply = function(inst)
                ...
                if inst.components.health then
                    inst.old_absorb = inst.components.health.absorb
                    inst.components.health.absorb = math.max(inst.components.health.absorb, 0.25)
                end
                -- AOE攻击效果
                inst.bee_queen_aoe = true
            end,
            on_remove = function(inst)
                if inst.components.health and inst.old_absorb then
                    inst.components.health.absorb = inst.old_absorb
                    inst.old_absorb = nil
                end
                inst.bee_queen_aoe = false
            end,
        }
    },
    ...
    klaus = {
        ...
        effects = {
            ...
            on_apply = function(inst)
                inst.klaus_revive_ready = true
                -- 监听死亡事件
                inst.klaus_death_listener = function(inst, data)
                    if inst.klaus_revive_ready then
                        inst.klaus_revive_ready = false
                        -- 复活逻辑
                        inst:DoTaskInTime(1, function()
                            if inst.components.health and inst.components.health:IsDead() then
                                inst.components.health:SetPercent(0.5)
                                inst.components.talker:Say("克劳斯的力量让我重生!")
                ...
                inst:ListenForEvent("death", inst.klaus_death_listener)
            end,
            on_remove = function(inst)
                inst.klaus_revive_ready = false
                if inst.klaus_death_listener then
                    inst:RemoveEventCallback("death", inst.klaus_death_listener)
                    inst.klaus_death_listener = nil
                end
            end,
        }
    },
    ...
    ancient_fuelweaver = {
        name = "远古守护者之力",
        description = "攻击时生成暗影触手造成额外伤害",
        duration = 5 * 480,
        icon = "fuelweaver_buff",
        effects = {
            tags = { "shadow_striker" },
            bonuses = {
                shadow_tentacles = true,
                extra_damage = 34,
            },
            on_apply = function(inst)
                inst.shadow_strike_enabled = true
                -- 监听攻击事件
                inst.shadow_attack_listener = function(inst, data)
                    if data and data.target and inst.shadow_strike_enabled then
                        -- 生成暗影触手
                        local x, y, z = data.target.Transform:GetWorldPosition()
                        local tentacle = SpawnPrefab("shadowtentacle")
                        if tentacle then
                            tentacle.Transform:SetPosition(x, y, z)
                            tentacle:DoTaskInTime(0.5, function()
                                if data.target and data.target.components.health then
                                    data.target.components.health:DoDelta(-34)
                ...
                inst:ListenForEvent("onhitother", inst.shadow_attack_listener)
            end,
            on_remove = function(inst)
                inst.shadow_strike_enabled = false
                if inst.shadow_attack_listener then
                    inst:RemoveEventCallback("onhitother", inst.shadow_attack_listener)
                    inst.shadow_attack_listener = nil
                end
            end,
        }
    },
    ...
    toadstool = {
        name = "毒菌蟾蜍之力",
        description = "免疫催眠，物品腐烂速度减慢10倍",
        duration = 5 * 480,
        icon = "toadstool_buff",
        effects = {
            tags = { "sleep_immune", "preservation_master" },
            immunities = { "sleep", "spoilage" },
            bonuses = {
                spoilage_rate = 0.1, -- 腐烂速度减慢10倍
                sleep_immunity = true,
            },
            on_apply = function(inst)
                if inst.components.grogginess then
                    inst.components.grogginess:SetResistance(999)
                end
                inst.preservation_active = true
            end,
            on_remove = function(inst)
                if inst.components.grogginess then
                    inst.components.grogginess:SetResistance(3)
                end
                inst.preservation_active = false
            end,
        }
    },
    ...
    stalker = {
        name = "织影者之力",
        description = "获得骨甲的无敌效果",
        duration = 5 * 480,
        icon = "stalker_buff",
        effects = {
            tags = { "bone_armor" },
            bonuses = {
                periodic_invincibility = true,
                invincible_duration = 0.5,
                invincible_cooldown = 5,
            },
            on_apply = function(inst)
                inst.bone_armor_timer = 0
                inst.bone_armor_task = inst:DoPeriodicTask(0.1, function()
                    inst.bone_armor_timer = inst.bone_armor_timer + 0.1
                    if inst.bone_armor_timer >= 5 then
                        inst.bone_armor_timer = 0
                        -- 激活无敌
                        if inst.components.health then
            ...
            on_remove = function(inst)
                if inst.bone_armor_task then
                    inst.bone_armor_task:Cancel()
                    inst.bone_armor_task = nil
                end
                inst.bone_armor_timer = nil
            end,
        }
    },
    ...
    malbatross = {
        ...
        effects = {
            ...
            on_apply = function(inst)
                inst.water_walking = true
                -- 修改locomotor以支持水上行走
                if inst.components.locomotor then
                    inst.old_platform_hopping = inst.components.locomotor.allow_platform_hopping
                    inst.components.locomotor:SetAllowPlatformHopping(true)
                end
            end,
            on_remove = function(inst)
                inst.water_walking = false
                if inst.components.locomotor and inst.old_platform_hopping ~= nil then
                    inst.components.locomotor:SetAllowPlatformHopping(inst.old_platform_hopping)
                    inst.old_platform_hopping = nil
                end
            end,
        }
    },
    ...
    crabking = {
        name = "帝王蟹之力",
        description = "攻击具有吸血效果",
        duration = 5 * 480,
        icon = "crab_king_buff",
        effects = {
            tags = { "life_steal" },
            bonuses = {
                life_steal_percent = 0.25, -- 25%吸血
                claw_attacks = true,
            },
            on_apply = function(inst)
                inst.life_steal_active = true
                inst.life_steal_listener = function(inst, data)
                    if data and data.target and data.damage and inst.life_steal_active then
                        local heal = math.floor(data.damage * 0.25)
                        if heal > 0 and inst.components.health then
                            inst.components.health:DoDelta(heal, true)

                            -- 吸血特效
                            local fx = SpawnPrefab("healthregenbuff_fx")
                            if fx then
                                fx.Transform:SetPosition(inst.Transform:GetWorldPosition())
                            end
                        end
                ...
                inst:ListenForEvent("onhitother", inst.life_steal_listener)
            end,
            on_remove = function(inst)
                inst.life_steal_active = false
                if inst.life_steal_listener then
                    inst:RemoveEventCallback("onhitother", inst.life_steal_listener)
                    inst.life_steal_listener = nil
                end
            end,
        }
    },
    ...
    alterguardian_phase3 = {
        name = "天体英雄之力",
        description = "自带照明，攻击时有虚灵协助",
        duration = 5 * 480,
        icon = "celestial_buff",
        effects = {
            tags = { "celestial_champion", "enlightened" },
            bonuses = {
                permanent_light = true,
                gestalt_helpers = true,
                sanity_immunity = true,
            },
            on_apply = function(inst)
                -- 永久照明
                if inst.components.playerlight then
                    inst.celestial_light = SpawnPrefab("minerhatlight")
                    if inst.celestial_light then
                        inst.celestial_light.entity:SetParent(inst.entity)
                        inst.celestial_light.Light:SetRadius(8)
                        inst.celestial_light.Light:SetIntensity(0.8)
                        inst.celestial_light.Light:SetColour(180 / 255, 195 / 255, 255 / 255)
                    end
                end

                -- 虚灵协助攻击
                inst.gestalt_attack_listener = function(inst, data)
                    if data and data.target then
                        -- 生成虚灵协助攻击
                        local gestalt = SpawnPrefab("gestalt")
                        if gestalt then
                            gestalt.Transform:SetPosition(inst.Transform:GetWorldPosition())
                            gestalt:DoTaskInTime(0.5, function()
                                if data.target and gestalt and gestalt:IsValid() then
                                    if data.target.components.health then
                                        data.target.components.health:DoDelta(-20)
                                    end
                                end
                                gestalt:Remove()
                            end)
                ...
                inst:ListenForEvent("onhitother", inst.gestalt_attack_listener)

                -- san值低不受影响
                inst.celestial_sanity_protection = true
            end,
            on_remove = function(inst)
                if inst.celestial_light then
                    inst.celestial_light:Remove()
                    inst.celestial_light = nil
                end

                if inst.gestalt_attack_listener then
                    inst:RemoveEventCallback("onhitother", inst.gestalt_attack_listener)
                    inst.gestalt_attack_listener = nil
                end

                inst.celestial_sanity_protection = false
            end,
        }
    },
}
...
-- Boss能力管理器类
local BossAbilityManager = Class(function(self, inst)
    self.inst = inst
    self.current_ability = nil
    self.ability_task = nil
    self.abilities_unlocked = {}
end)

function BossAbilityManager:ApplyAbility(boss_name)
    local ability_data = BOSS_ABILITIES[boss_name]
    if not ability_data then
        return false
    end

    -- 记录解锁的能力
    self.abilities_unlocked[boss_name] = true

    -- 移除当前能力
    self:RemoveCurrentAbility()

    -- 应用新能力
    self.current_ability = boss_name
    local effects = ability_data.effects

    -- 添加标签
    if effects.tags then
        for _, tag in ipairs(effects.tags) do
            self.inst:AddTag(tag)
        end
    end

    -- 应用效果
    if effects.on_apply then
        effects.on_apply(self.inst)
    end

    -- 显示获得能力的消息
    self.inst.components.talker:Say("获得了" .. ability_data.name .. "！")

    -- 播放获得能力的特效
    self:PlayAbilityFX(boss_name, "gain")

    -- 设置到期任务
    self.ability_task = self.inst:DoTaskInTime(ability_data.duration, function()
        self:RemoveCurrentAbility()
        self.inst.components.talker:Say(ability_data.name .. "消失了...")
    end)

    return true
end

function BossAbilityManager:RemoveCurrentAbility()
    if not self.current_ability then
        return
    end

    local ability_data = BOSS_ABILITIES[self.current_ability]
    if ability_data then
        ...
        -- 移除效果
        if effects.on_remove then
            effects.on_remove(self.inst)
        end

        -- 播放失效特效
        -- self:PlayAbilityFX(self.current_ability, "lose")
    end

    -- 清理任务
    if self.ability_task then
        self.ability_task:Cancel()
        self.ability_task = nil
    end
...
function BossAbilityManager:PlayAbilityFX(boss_name, fx_type)
    local fx_name = fx_type == "gain" and "deer_ice_burst" or "collapse_small"
    ...
    if fx then
        ...
        -- 根据boss类型设置不同颜色
        local colors = {
            spiderqueen = { 0.5, 1, 0.5, 1 }, -- 绿色
            dragonfly = { 1, 0.5, 0.2, 1 }, -- 橙色
            deerclops = { 0.7, 0.9, 1, 1 }, -- 冰蓝色
            -- 更多颜色...
        }

        local color = colors[boss_name]
        if color and fx.AnimState then
            fx.AnimState:SetMultColor(color[1], color[2], color[3], color[4])
        end
    end
end

function BossAbilityManager:GetCurrentAbility()
    return self.current_ability
end

function BossAbilityManager:GetRemainingTime()
    if self.ability_task then
        return self.ability_task.elapsed_time or 0
    end
    return 0
end

function BossAbilityManager:HasUnlocked(boss_name)
    return self.abilities_unlocked[boss_name] == true
end

function BossAbilityManager:OnSave()
    local data = {
    ...
    if self.ability_task then
        data.remaining_time = self.ability_task.elapsed_time
    end

    return data
end

function BossAbilityManager:OnLoad(data)
    if data then
        self.abilities_unlocked = data.abilities_unlocked or {}

        if data.current_ability and data.remaining_time then
            -- 恢复能力
            self:ApplyAbility(data.current_ability)
            -- 重新设置剩余时间
            if self.ability_task then
                self.ability_task:Cancel()
                self.ability_task = self.inst:DoTaskInTime(data.remaining_time, function()
                    self:RemoveCurrentAbility()
                end)
            end
        end
...
AddPlayerPostInit(function(inst)
    if inst.prefab == "vox" then
        inst.boss_ability_manager = BossAbilityManager(inst)

        -- 监听Boss击杀事件
        inst:ListenForEvent("killed", function(inst, data)
            if data and data.victim and data.victim:HasTag("epic") then
                local boss_name = data.victim.prefab
                -- 特殊处理一些boss名称映射
                local boss_mapping = {
                    moose_goose = "moose",
                    bearger = "bearger",
                ...
                local actual_boss = boss_mapping[boss_name] or boss_name
                inst.boss_ability_manager:ApplyAbility(actual_boss)
            end
        end)
        ...
        inst.OnSave = function(inst, data)
            ...
            data.boss_abilities = inst.boss_ability_manager:OnSave()
        end
        ...
        inst.OnLoad = function(inst, data)
            ...
            if data and data.boss_abilities then
                inst.boss_ability_manager:OnLoad(data.boss_abilities)
            end
        end
    end
end)
...
AddComponentPostInit("perishable", function(self, inst)
    ...
    function self:StartPerishing()
        ...
        if owner and owner:IsValid() and owner:HasTag("vox") and owner.preservation_active then
            -- 腐烂速度减慢10倍
            self.perishremainingtime = self.perishremainingtime * 10
        end
    end
end)

-- 处理蜘蛛网速度加成
AddPrefabPostInit("web", function(inst)
    local old_OnSteppedOn = inst.components.groundcover and inst.components.groundcover.onSteppedOn
    if old_OnSteppedOn then
        inst.components.groundcover.onSteppedOn = function(inst, obj)
            if obj and obj:HasTag("vox") and obj:HasTag("webwalker") then
                -- 在蛛网上获得速度加成
                if obj.components.locomotor then
                    obj.components.locomotor:SetExternalSpeedMultiplier(obj, "spider_web_temp", 1.25)
                    obj:DoTaskInTime(0.5, function()
                        obj.components.locomotor:RemoveExternalSpeedMultiplier(obj, "spider_web_temp")
                    end)
                end
            else
                old_OnSteppedOn(inst, obj)
            end
        end
    end
end)
...
-- 添加恶魔饥饿条到VOX角色的HUD
-- 在modmain.lua中添加UI整合代码
local function AddVoxUI()
    -- 恶魔饥饿条UI
    AddClassPostConstruct("widgets/statusdisplays", function(self)
        if self.owner and self.owner:HasTag("vox") then
            -- 创建恶魔饥饿条
            local DemonHungerBadge = require("widgets/demonhungerbadge")
            self.demonhunger = self:AddChild(DemonHungerBadge(self.owner))

            -- 设置位置（在饥饿条旁边）
            if self.hunger then
                self.demonhunger:SetPosition(110, 0, 0)
            end

            -- 监听恶魔饥饿值变化
            self.inst:ListenForEvent("demonhungerdelta", function(owner, data)
                if self.demonhunger then
                    self.demonhunger:SetPercent(data.newpercent)
                end
            end, self.owner)
        end
    end)
...
-- 检测猴子位置的按键绑定
-- 替换现有的猴子感应系统
local MONKEY_SENSE_KEY = GetModConfigData("monkey_sense_key") or KEY_H
    ...
    if #monkeys > 0 then
        ...
        if nearest then
            ...
            if nearestDist < 10 then
                distance_msg = STRINGS.CHARACTERS.VOX.ANNOUNCE_MALOU_INDUCTION_NEAR or "它们好像就在我身边!"
            elseif nearestDist < 20 then
                distance_msg = STRINGS.CHARACTERS.VOX.ANNOUNCE_MALOU_INDUCTION_CLOSE or "很近了."
            elseif nearestDist < 40 then
                distance_msg = STRINGS.CHARACTERS.VOX.ANNOUNCE_MALOU_INDUCTION_FAR or "猴子离我有点远."
            else
                distance_msg = STRINGS.CHARACTERS.VOX.ANNOUNCE_MALOU_INDUCTION_EXFAR or "太远了."
            end
            ...
            if nearestDist < 20 then
                ...
                player:DoTaskInTime(0.1, function()
                    -- 创建临时标记效果
                    local marker = SpawnPrefab("minimap_marker")
                    if marker then
                        marker.Transform:SetPosition(mx, 0, mz)
                        marker:DoTaskInTime(5, function() marker:Remove() end)
                    end
                end)
            end
        end
    else
        player.components.talker:Say(STRINGS.CHARACTERS.VOX.ANNOUNCE_MALOU_INDUCTION_FAIL or "感应不到任何猴子.")
    end
end

-- 按键绑定
TheInput:AddKeyDownHandler(MONKEY_SENSE_KEY, function()
    local player = ThePlayer
    if player and player:IsValid() then
        DoMonkeySense(player)
    end
end)
