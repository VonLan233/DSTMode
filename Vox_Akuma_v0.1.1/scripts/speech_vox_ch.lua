return {

	ANNOUNCE_NIGHTMARE = "我不想再回忆起那段噩梦了! ",
	ANNOUNCE_MALOU_INDUCTION_NEAR = "它们好像就在我身边! ",
	ANNOUNCE_MALOU_INDUCTION_CLOSE = "很近了.",
	ANNOUNCE_MALOU_INDUCTION_FAR = "猴子离我有点远.",
	ANNOUNCE_MALOU_INDUCTION_EXFAR = "太远了.",
	ANNOUNCE_MALOU_INDUCTION_FAIL = "感应不到任何猴子.",
	ANNOUNCE_BE_KBC="不! 不要! 我不想的! ",
	ANNOUNCE_AFTER_KBC="变小是多么得令人安心.",


	ACTIONFAIL_GENERIC = { "我做不到.", "我不能这样做.", },
	
	ANNOUNCE_BOOMERANG = 
	{
		"哎呦! 好痛啊!",
		"我是笨蛋",
	},
	ANNOUNCE_LIGHTNING = "轰-隆!!\n闪电差点吓到我了!",
	ANNOUNCE_CHARLIE = "谁在那?",
	ANNOUNCE_CHARLIE_ATTACK = "不！你杀不死我的!",
	ANNOUNCE_COLD = "好冷.",
	ANNOUNCE_HOT = "好热啊,我想上凉快的地方!",
	ANNOUNCE_CRAFTING_FAIL = "我缺乏必要的材料.",
	ANNOUNCE_DUSK = "天快黑了...",
	ANNOUNCE_EAT =
	{
		GENERIC = "啊呣啊呣啊呣!",
		PAINFUL = "即便是恶魔也不想吃这个.",
		SPOILED = "不要吃那个! 呕! ",
		STALE = "是陈旧的吗?",
		INVALID = "我拒绝.",
	},
	ANNOUNCE_ENTER_DARK =
	{
		"黑暗中有希望",
		"Kindred，别怕，快来我身边...",
	},
	ANNOUNCE_ENTER_LIGHT =
	{
		"这儿有光!",
		"终于亮了",
		"我从黑暗里回来了.",
	},
	ANNOUNCE_HOUNDS = "小狗狗要来了.",
	ANNOUNCE_HUNGRY =
	{
		"不，别又……",
		"我最担心的事又要发生了.",
		"不，我不要再吃了!",
	},
	ANNOUNCE_HUNT_BEAST_NEARBY = "Kindred!离远点!",
	ANNOUNCE_HUNT_LOST_TRAIL = "跟丢了...",
	ANNOUNCE_HUNT_LOST_TRAIL_SPRING = "跟丢了...",
	ANNOUNCE_INV_FULL = { "我拿不下了!", "我放不下更多的东西了!", },
	ANNOUNCE_KNOCKEDOUT = "嗷!",
	ANNOUNCE_NOHUNGERSLEEP = "我会在睡梦中又失手杀死谁吗?",
	ANNOUNCE_PECKED = "你在考验我的耐心.",
	ANNOUNCE_QUAKE = "什么声音?",
	ANNOUNCE_THORNS =
	{
		"嗷! 荆棘, 好痛!",
		"真的好痛!",
	},
	ANNOUNCE_TORCH_OUT =
	{
		"天黑了!",
		"火灭了...",
	},
	ANNOUNCE_TRAP_WENT_OFF = "重新设置一下.",
	ANNOUNCE_WORMHOLE = "这是一个奇怪的冒险!",
	ANNOUNCE_CANFIX = "那个\n我认为我可以修好它!",
	ANNOUNCE_ACCOMPLISHMENT = "所有这一切是短暂的.",
	ANNOUNCE_ACCOMPLISHMENT_DONE = "有一天,终究会被遗忘.",
	ANNOUNCE_INSUFFICIENTFERTILIZER = "我感觉它有更多用途!",
	ANNOUNCE_LIGHTNING_DAMAGE_AVOIDED = "差点要跟Kindred永别了.",
	ANNOUNCE_BEES = "别蛰脸啊!!!!",
	ANNOUNCE_DEERCLOPS = "Kindred,你先走,我来干掉它!",
	ANNOUNCE_FREEDOM = "自由!",
	ANNOUNCE_MOSQUITOS = "可恶的蚊子! ",
	ANNOUNCE_NODANGERSLEEP = "有杀气，不能睡",--
	ANNOUNCE_NODAYSLEEP = "太亮了,睡不着.",
	ANNOUNCE_NODAYSLEEP_CAVE = "我还不困.",
	ANNOUNCE_NOSLEEPONFIRE = "还有很长的路要走.",
	ANNOUNCE_NODANGERSIESTA = "有杀气，不能睡",--
	ANNOUNCE_NONIGHTSIESTA = "我真的很想在帐篷里睡觉.",
	ANNOUNCE_NONIGHTSIESTA_CAVE = "我真的很想在帐篷里睡觉.",
	ANNOUNCE_NOHUNGERSIESTA = "我的肚子快饿疯了, 我现在不能打盹.",
	ANNOUNCE_SHELTER = "让我为Kindred撑伞.",
	ANNOUNCE_BURNT = "噢! 地狱之火!",
	ANNOUNCE_TOOL_SLIP = "好湿，像Kindred一样.",
	ANNOUNCE_DAMP = "有点湿...",--
	ANNOUNCE_WET = "我衣服湿了.",
	ANNOUNCE_WETTER = "到处都是水.",
	ANNOUNCE_SOAKED = "咕噜咕噜，要淹死了.",
	BATTLECRY =
	{
		GENERIC = "来啊,让我咬一口.",--
		PIGMAN = "小猪猪,缴蹄不杀!",--
		PREY = "我要逮到你!",--
		SPIDER = "别动!变成肉丸滚过来!",--
		SPIDER_WARRIOR = "别动! 变成肉丸自己滚过来!",--
		HOUND = "我要拔了你的狗牙! 小狗狗!",--
		FIREHOUND = "嘿!小火狗,你来咬我呀!",
		ICEHOUND = "嘿!小白狗来玩打雪仗吗!",
		WARG = "可怕的巨狼来了!战斗!战斗!",--
	},
	COMBAT_QUIT =
	{
		GENERIC = "为了Kindred,要活下去.",
	},
	DESCRIBE =
	{
		BROKENTHRONE = "曾经是我的王座, 但现在坏了, 我得修复它.",
		THRONE = "可以重新坐上去了! ",
		GHOST_KINDRED = "对不起……都是我害的……",
		PET_KINDRED = "我的宝贝甜心小可爱! 快让我摸摸你亲亲你! ",
		ENVELOPE_NC = "给我的信? ",
		LETTER_NC =
 		{
			"如果有一天我不见了，请不要怀疑是我背叛了你。我只是死了.",
			"我从未死亡，只是用另一种方式伴你同行，你永远不会是独自一人.",
			"铭记过去，而不是逃避过去，证明了将我们联系在一起的真诚.",
			"我破碎的灵魂等待着收留它的神谕，我终于等到了你，带走它吧.",
			"我会一直为你等待.",
			"虽然再也没有来生，但是能一直像这样和领主待在一起我也很开心.",
			"希望Milord可以一直开心，做想做的事情.",
			"我正在被灼痛着，是Milord正在遭受什么吗？",
			"永远支持你！爱你<3",
			"我并不期待来生 这里很温暖 这里便是我的归处.",
			"Sir这地方怎么这么黑，灯的开关在哪里.",
			"我感觉你完整了我.",
			"我希望你在吃了我之后能感觉到饱腹，请记住我永远爱你.",
			"外面的天气如何？",
			"你不是我的神，你是我的猴子，我爱你.",
			"亲爱的Milord，你辛苦了好些年.",
			"如果杀戮是恶魔的命运，我们或许可以一起改写星轨既定的结局.",
			"祝你今天开心.",
			"做你自己，有我爱你.",
			"已经过了太久也发生了太多但是你依旧是我的阳光.",
			"未来是未知的，让我们潜入深海，牵紧我的手，我们会找到勇气去直面一切.",
			"没有关系，未来可期.",
			"我自知时日无多，从今往后，每当有风吹拂过你，就是我在想你.",
			"由爱而生的勇气，足以抵抗世间所有的孤独.",
			"远着美梦永无尽头.",
			"谢谢你坚持这么久，此处给恶魔领主一个拥抱和额头吻.",

		},
		HAORI = "我的一位Kindred送我的, 嘿嘿.",
		KATANA = "刀下亡魂在被灼烧, 希望不是太痛.",
		ONIGIRI = "老朋友, 又想偷我的酒吗? ",
		CONTAINER_NC = "我用来存储灵魂的胃.",
	
		GLOMMER =
		{
			"一个健谈的小东西!",
			"啊呜! 小小的翅膀.",
			"我要保护好它,就像保护Kindred一样.",
		},
        GLOMMERFLOWER = 
        {
        	GENERIC = "夜来香.",
        	DEAD = "即使是花在夜晚也会死去.",
        },
        GLOMMERWINGS = "噢, 可怜的小东西.",--
        GLOMMERFUEL = "上好的燃料!",--
        BELL = "我离开后,你会记得我吗?",--
        STATUEGLOMMER = 
        {	
        	GENERIC = "我可以百分之百的肯定,这雕像是活的!",
        	EMPTY = "也许... 我杀了它...",
    	},
		WEBBERSKULL = "噢, 你这个可怜的小东西. 让我送你一朵花.",--
		MOLE =
		{
			HELD = "可以打地鼠吗?!",--
			UNDERGROUND = "呆在那里, 离开这个可怕的地方.",
			ABOVEGROUND = "你应该回到黑暗.",
		},
		MOLEHILL = "来啊, 来啊, 到地上来.",
		MOLEHAT = "好耶!夜视.",
		NIGHTSTICK =
		{
			"晴天霹雳!",
			"哇哦, 电击!",
		},
		RABBITHOUSE=
		{
			GENERIC = "这并不像看起来那么实用.",
			BURNT = "它没有煮好.",
		},
		TURF_DECIDUOUS = "漂亮的地皮.",
		TURF_SANDY = "一些地皮.",
		TURF_BADLANDS = "一些地皮.",
		BEARGER =
		{
			"哇呀呀呀! 这是吃货熊拆迁,快跑!",
			"有人来拆我们的家了,Kindred!",
		},
		BEARGERVEST = "耶! 这会让我看起来很蓬松!",--
		ICEPACK = "冰冰凉.",
		BEARGER_FUR =
		{
			"我爱皮草! "
		},
		BIGFOOT = "哦, 你是个很好的朋友. 如果你离我远点的话.",--
		BONESHARD = "犀利尖锐的骨头.",
		BUZZARD =
		{
			"丑秃鹫!",
			"它们爱吃腐烂的肉.",
		},
		CACTUS = 
		{
			GENERIC = "Kindred,当心刺!",
			PICKED = "烤烤更好吃...",
		},
		CACTUS_MEAT_COOKED = "我来切开.",
		CACTUS_MEAT = "仙人掌肉!它仍然很危险.",
		CACTUS_FLOWER = "如果我们能从这朵花中看到奇迹,\n我们的生活将会改变.",--
		COLDFIRE =
		{
			EMBERS = "黑暗中若隐若现.",
			GENERIC = "我也许这个夜晚生存!",
			HIGH = "冰火烧的太旺了!",
			LOW = "火焰弱了.",
			NORMAL = "这是一个奇怪的火.",
			OUT = "这不是一个好兆头.",
		},
		CATCOON =
		{
			"喵呜呜呜呜! 可爱的猫咪!",
		},
		CATCOONDEN = 
		{
			GENERIC = "据说，猫有九条命.",
			EMPTY = "所有的猫猫都走了. 它是空的.",
		},
		CATCOONHAT = "嗨!你看! 我像一只狸猫!",--
		COONTAIL = "啊哦,发生了惨剧.",
		COOKPOT =
		{
			COOKING_LONG = "快点,Kindred等着吃呢.",
			COOKING_SHORT = "需要油.",
			DONE = "完成, 食物.",
			EMPTY = "跟我没有灵魂一样.",
			BURNT = "操!",
		},
		EYEBRELLAHAT = "现在你不能流泪.",--
		ARMORDRAGONFLY = "这样就不怕火了.",
		DRAGON_SCALES = "剧烈的疼痛.",
		DRAGONFLYCHEST = "大箱子.",
		DECIDUOUSTREE = 
		{
			BURNING = "它明亮地燃烧.",
			BURNT = "用完了.",
			CHOPPED = "一切都死了.",
			GENERIC = "这棵橡树真漂亮. 我最爱吃烤熟的橡果啦.",
			POISON = "真奇特!",
		},
		ACORN_COOKED = "咦? 这是松鼠的食物.",
		BIRCHNUTDRAKE = "他们不是我的朋友! 但是, 太可爱了!",
		FARMPLOT =
		{
			GENERIC = "我要播下什么种子?",
			GROWING = "绿叶从新泥中出来了.",
			NEEDSFERTILIZER = "这个星球很荒凉.",
			BURNT = "我是在饥荒的世界里吗.",
		},
		COLDFIREPIT =
		{
			EMBERS = "黑暗中若隐若现.",
			GENERIC = "我会在夜晚幸存的!",
			HIGH = "像地狱火一样耀眼!",
			LOW = "火焰弱了.",
			NORMAL = "这是一个奇怪的火焰.",
			OUT = "这不是个好的信号.",
		},
		FIRESUPPRESSOR = 
		{	
			ON = "夏天会下冰雹吗?",--
			OFF = "它好冰冷.",
			LOWFUEL = "它在烟雾上跑.",
		},
		ICEHAT = "思维冻结了!",--
		LIGHTNINGGOAT = 
		{
			GENERIC = "如果他们都会被闪电击中是会发生什么?",--
			CHARGED = "哇!电灯耶!",
		},
		LIGHTNINGGOATHORN = "一些静电.",
		GOATMILK = "我不需要喝它来长高",--
		MEATRACK =
		{
			DONE = "可以吃啦.",
			DRYING = "我等不及了!",--
			DRYINGINRAIN = "下雨要保持干燥.",
			GENERIC = "我可以把肉晾在这里.",
			BURNT = "它永远的晾干了.",
		},
		MERMHEAD = 
		{
			GENERIC = "有股我讨厌的味道!",--
			BURNT = "终于不用再闻到了.",
		},
		MERMHOUSE = 
		{
			GENERIC = "我不能进去,我也不想去.",--
			BURNT = "火焰更大了.",
		},
		FLOWERSALAD = "夏季美食?",
        ICECREAM =
		{
			"我喜欢吃冰淇淋!",
		},
        WATERMELONICLE = "这瓜好甜.",
        TRAILMIX = "不够吃.",
        HOTCHILI = "这种程度不算什么",--
        GUACAMOLE = "阴森. 我喜欢.",
		MOOSE = "这是我见过的最大的鸭子.",
		MOOSEEGG = "我喜欢这个蛋.",--
		MOSSLING = "哇, 你这可爱的小鸭子!",
		FEATHERFAN = "哇哦, 这是我见过最大的羽毛!",--
		GOOSE_FEATHER = "哈哈哈! 停住! 好痒!",--
		STAFF_TORNADO = "我掌握了龙卷风的力量!",--
		PIGHEAD = 
		{	
			GENERIC = "你知道中国习俗吗, 人们过年用猪头供奉神仙?",--
			BURNT = "它被烧毁了.",
		},
		PIGHOUSE =
		{
			FULL = "我打赌里面很暖和...",--
			GENERIC = "这真是个糟糕的房子!",--
			LIGHTSOUT = "现在就我一个人了.",
			BURNT = "万物皆会消失没有永恒.",
		},
		FERTILIZER = "一桶污垢.",
		RAINOMETER = 
		{	
			GENERIC = "我希望明天阳光灿烂!",--
			BURNT = "那意味着明天没有太阳, 是吗?",
		},
		RAINCOAT = "我想跳进水坑里!",--
		RAINHAT = "如果下雨淋湿的话,那一定很有趣?",
		RESEARCHLAB = 
		{	
			GENERIC = "我想我可以做一些很酷的事.",--
			BURNT = "烧的一塌糊涂.",
		},
		RESEARCHLAB2 = 
		{
			GENERIC = "我要做些另人吃惊的事.",--
			BURNT = "烧得一塌糊涂.",
		},
		RESEARCHLAB3 = 
		{
			GENERIC = "皮影戏吗!好看吗?",--
			BURNT = "烧得一塌糊涂.",
		},
		RESEARCHLAB4 = 
		{
			GENERIC = "什么!? 新玩具!? 在哪儿呢!?",--
			BURNT = "玩火自焚.",
		},
		RESURRECTIONSTATUE = 
		{
			GENERIC = "尽情去战斗吧!",--
			BURNT = "不!!!!!",
		},
		ROCK_ICE = 
		{
			GENERIC = "嘿, 这里好冷.",--
			MELTED = "结冰了.",
		},
		ROCK_ICE_MELTED = "化了.",
		ICE = "旺旺碎碎冰.",
        REFLECTIVEVEST = "完美背心.",
		HAWAIIANSHIRT = "看上去挺时髦?",--
		TENT = 
		{
			GENERIC = "Kindred来和我一起!",
			BURNT = "地狱之火又一次烧掉了我的住所.",
		},
		SIESTAHUT = 
		{
			GENERIC = "这能让我有一个舒服的睡一个午觉.",
			BURNT = "地狱之火将会开始燃烧.",
		},
		TRANSISTOR = "电器是什么?",
		TREASURECHEST = 
		{
			GENERIC = "为了我的财富.",
			BURNT = "他不能是灰烬.",
		},
		TUMBLEWEED = "过来, 风滚草! 我需要你!",--
		GRASS_UMBRELLA = "过来Kindred,让我为你撑伞.",--
		UNIMPLEMENTED = "它不完整, 如生活一样.",
		WALL_HAY = 
		{	
			GENERIC = "这只是防御的建议.",
			BURNT = "挡下来.",
		},
		WALL_WOOD = 
		{
			GENERIC = "事不宜迟.",
			BURNT = "它一定会来的.",
		},
		WARG = "噢, 别别别. 我一定要和狗狗做朋友.",--
		WATERMELON = "它大部分是水. 有什么大不了的?",
		WATERMELON_COOKED = "现在是温水.",
		WATERMELONHAT = "妈妈不会想见到我这样的.",--
		WINTEROMETER = 
		{
			GENERIC = "为什么这是衡量我的死亡率?",
			BURNT = "这意味着我终有一天会死亡在这里?",
		},
		HOMESIGN = 
		{
			GENERIC = "它说什么? 你明天会死的. 开玩笑啦.",--
			BURNT = "的确瞬息万变.",
		},
		BEEBOX =
		{
			GENERIC = "蜜蜂很勤劳,它们会把蜂蜜放在里面.",
			READY = "它们艰苦的工作已经取得丰硕的成果!",
			FULLHONEY = "他们的辛劳取得了成果!",
			NOHONEY = "去采蜜, 小蜜蜂!",
			SOMEHONEY = "采蜜中.",
			BURNT = "焦蜜.",
		},
		LIVINGTREE = "这棵树好可怕,我好像在万圣节看到过?",
		ICESTAFF = "我讨厌寒冷!",--
		WORMLIGHT = "我是唯一一个可以在晚上发光的恶魔!",--
		WORM =
		{
		    PLANT = "看来是世外桃源.",
		    DIRT = "有什么东西在这里?",
		    WORM = "他们来自黑暗.",
		},
		EEL = "那令人恶心的东西是什么?",--
		EEL_COOKED = "烤过也还是很腥",--
		UNAGI = "我才不要吃那个!",--
		EYETURRET = "你究竟在看什么?",--
		EYETURRET_ITEM = "嘿,你好 小家伙!",--
		MINOTAURHORN = "一个曾经伟大的野兽的遗骸.",
		MINOTAURCHEST = "这是黄金和钻石的气味!",--
		THULECITE_PIECES = "一种强力材料的碎片.",
		POND_ALGAE = "一些藻类.",
		GREENSTAFF = "我能看到内在的强大力量.",
		POTTEDFERN = "噢, 可爱的小东西! 你看起来需要一个永远的家.",--

		THULECITE = "我能感受到强烈的神秘力量在里头.",--
		ARMORRUINS = "没有人能敢碰我!",--
		RUINS_BAT = "这个神奇的蝙蝠会带给我一个朋友.",--
		RUINSHAT = "白马王子在哪?",--
		NIGHTMARE_TIMEPIECE = --Keeps track of the nightmare cycle
		{
			CALM = "我不知道什么潜伏在阴影中.",	--calm phase
			WARN = "我能感觉到窥视的眼睛.",	--Before nightmare
			WAXING = "它们接近了.", --Nightmare Phase first 33%
			STEADY = "它们把我包围了!", --Nightmare 33% - 66%
			WANING = "我认为它们离开了.", --Nightmare 66% +
			DAWN = "都结束了.", --After nightmare
			NOMAGIC = "我很孤独.", --Place with no nightmare cycle.
		},
		BISHOP_NIGHTMARE = "这场战争时间赢了.",
		ROOK_NIGHTMARE = "岁月是把杀猪刀.",
		KNIGHT_NIGHTMARE = "又一次败给时间.",
		MINOTAUR = "为什么你这么小气? 我们是朋友吗?",-- --Monster in labyrinth
		SPIDER_DROPPER = "鬼鬼祟祟的小东西! 你在偷窥什么!",-- --White spider that comes from the roof
		NIGHTMARELIGHT = "我不想做恶梦.",-- --Lights that activate during nightmare.
		RELIC = "都已是明日黄花.",	--Fixed relic
		RUINS_RUBBLE = "都已是明日黄花.",	--Broken relic
		MULTITOOL_AXE_PICKAXE = "这个东西有多种用途.",	--Works as axe and pickaxe
		GREENGEM = "美过头了.",
		ORANGESTAFF = "可不是我懒惰,而是它真的很好用.",-- --Teleports player.
		YELLOWAMULET = "现在黑暗也能离我远点了!",-- --Emits light, player walks faster.
		GREENAMULET = "节约是好文明!",	--Reduce cost of crafting

		SLURPER = "它看起来像我70岁玩过的玩具.",--
		SLURPER_PELT = "嗯, 这是头发吗?",--
		ARMORSLURPER = "不. 我不胖. 至少现在不是.",--
		ORANGEAMULET = "不用自己动手,真好!",
		YELLOWSTAFF = "温暖又明亮,让我想起Kindred.",
		YELLOWGEM = "神秘的宝石镶嵌在古城遗址.",
		ORANGEGEM = "这个让我感到很开心.",
		TELEBASE = 
		{
			VALID = "有能量在流动.",
			GEMS = "死气沉沉.",
		},
		GEMSOCKET = 
		{
			VALID = "它镶满了宝石.",
			GEMS = "你看它像什么.",
		},
		STAFFLIGHT = "一闪一闪亮晶晶.",--

        ANCIENT_ALTAR = "它身边充满死亡的气息.",
        ANCIENT_ALTAR_BROKEN = "我听到死亡的喘息了.",
        ANCIENT_STATUE = "呵, 这东西应该是可怕的?",--

        LICHEN = "怎样能让它快速成长?",
		CUTLICHEN = "转瞬即逝, 就像生活一样.",
		CAVE_BANANA = "猴子最爱这个,那我应该也是.",
		CAVE_BANANA_COOKED = "熟香蕉真美味\n隔壁猴子都馋哭了.",
		CAVE_BANANA_TREE = "可怜的猴子没香蕉吃了.",
		ROCKY = "吖, 可怜的岩石.",

		BLUEAMULET = "这个会冰冻我的心吗?",--
		PURPLEAMULET = "不,又是那些东西!",--
		TELESTAFF = "呀! 不好意思!",--
		MONKEY =
		{
			"猴子!吼!吼!吼!",
			"我的最爱!",
		},
		MONKEYBARREL = "猴子们的家,我也想住进去.",
		MONSTERLASAGNA = "呕,是恶魔也不想碰的食物.",--
		HOUNDSTOOTH="哦!这些牙齿还不如我的尖!",--
		ARMORSNURTLESHELL="看! 我有一个移动的家.",--
		BAT=
		{
			"它们傍晚醒来.",
			"不要碰它们! 我不想打...",
		},
		BATBAT = "蝙蝠很强,但是没啥影响.",--
		BATWING="我也可以飞,但不是现在.",--
		BATWING_COOKED="哈哈! 这看起来就像一个回旋镖.",--
		BEDROLL_FURRY="毛茸茸的很舒服.",
		BUNNYMAN="小兔子最讨厌肉了.",--
		FLOWER_CAVE="这个看起来很脆.",
		FLOWER_CAVE_DOUBLE="它看起来真脆.",
		FLOWER_CAVE_TRIPLE="它看起来真脆.",
		GUANO="可以用它浇灌我的植物.",
		LANTERN="照亮黑暗的路.",
		LIGHTBULB="有了它,我不在害怕黑暗了.",--
		MANRABBIT_TAIL="真可爱.",
		MUSHTREE_TALL  ="它会发光呢.",
		MUSHTREE_MEDIUM="它看起来像小精灵的房子.",
		MUSHTREE_SMALL ="有些小矮人生活在这里?",
		SLURTLE="慢慢爬吧.",
		SLURTLE_SHELLPIECES="破碎的梦想碎片.",
		SLURTLEHAT="我怀疑它会帮助我,就像帮助它原来的主人一样.",
		SLURTLEHOLE="当它们长大了就会被抛弃.",
		SLURTLESLIME="有点恶心.",
		SNURTLE="那一个有巨大的壳.",
		SPIDER_HIDER="有种你就别缩壳,出来让我打.",
		SPIDER_SPITTER="喷得到处都是,真恶心.",
		SPIDERHOLE="这里有蜘蛛出没.",
		STALAGMITE="大地放弃了它的恩惠.",
		STALAGMITE_FULL="大地放弃了它的恩惠.",
		STALAGMITE_LOW="大地放弃了它的恩惠.",
		STALAGMITE_MED="大地放弃了它的恩惠.",
		STALAGMITE_TALL="地球在翻转.",
		STALAGMITE_TALL_FULL="地球在翻转.",
		STALAGMITE_TALL_LOW="地球在翻转.",
		STALAGMITE_TALL_MED="地球在翻转.",

		TURF_CARPETFLOOR = "地毯地板.",
		TURF_CHECKERFLOOR = "方格地板.",
		TURF_DIRT = "污垢地皮.",
		TURF_FOREST = "森林草皮.",
		TURF_GRASS = "长草草皮.",
		TURF_MARSH = "沼泽地皮.",
		TURF_ROAD = "有速度加成,冲呀.",
		TURF_ROCKY = "岩石地皮.",
		TURF_SAVANNA = "热带草原草皮.",
		TURF_WOODFLOOR = "木质地板.",

		TURF_CAVE="祥云地皮.",
		TURF_FUNGUS="真菌地.",
		TURF_SINKHOLE="天坑地.",
		TURF_UNDERROCK="一些地皮.",
		TURF_MUD="浑浑......某处触角!",

		POWCAKE =
		{
			"嘿嘿嘿! 这不是食物!",
			"会死的...",
		},
		CAVE_ENTRANCE = 
		{
			GENERIC="这扇门之外还有另外一个世界.",--
			OPEN = "这是一扇通往地狱的门.",--
		},
        CAVE_EXIT = "嘿,小朋友!你能把我带上来吗?",--
		BOOMERANG = "死亡会通向地狱.",
		MAXWELLPHONOGRAPH = "我们两个谁更坏?",
		ABIGAIL = "这是双胞胎妹妹, 阿比盖尔.",
		PIGGUARD = "无聊的小猪...",--
		ADVENTURE_PORTAL = "嗅嗅. 我闻到了一个新的冒险!",--
		AMULET = "这将保护我和Kindred远离死亡和危险.",--
		ANIMAL_TRACK = "Kindred,你在这里吗...",--
		ARMORGRASS = "凑合用吧.",
		ARMORMARBLE = "好重,但是安全.",
		ARMORWOOD = "多准备点.",
		ARMOR_SANITY = "穿这个是要付出代价的.",
		ASH =
		{
			GENERIC = "所有这一切都留下了另一个美丽的火灾.",
			REMAINS_GLOMMERFLOWER = "不!",
			REMAINS_EYE_BONE = "对不起,我没照顾好它!",
			REMAINS_THINGIE = "这是之前烧过的...",
		},
		AXE = "工作和谋杀的工具.",
		BABYBEEFALO = "嘿!小家伙,你的妈妈呢?",--
		BACKPACK = "耶, 背上行囊去旅行!",--
		BACONEGGS = "早餐.",--
		BANDAGE = "我不会再受伤了.",
		BASALT = "别去管它,毫无意义.",
		BEARDHAIR = "我也有.",
		BEDROLL_STRAW = "一个人时我常常失眠.",
		BEE =
		{
			GENERIC = "我将摆脱那毒刺.",
			HELD = "别蛰脸!",
		},
		BEEFALO =
		{
			FOLLOWER = "你跑不过我的.",
			GENERIC = "它看上去呆呆的.",
			NAKED = "OOPS,跟我没关系.",
			SLEEPING = "趁现在!快剃了它的毛!",
		},
		BEEFALOHAT = "现在我可以伪装成他们的一员了.",
		BEEFALOWOOL = "有时, 它们也会感到冷的.",
		BEEHAT = "这将保护我的帅脸不被蛰.",
		BEEHIVE = "里面有好多蜂蜜.",
		BEEMINE = "它不分敌我",--
		BERRIES = "好吃的浆果.",
		BERRIES_COOKED = "很美味, 但是它们不会持续很长时间.",
		BERRYBUSH =
		{
			BARREN = "快长出来.",
			WITHERED = "它需要水或者肥料.",
			GENERIC = "一份小吃.",
			PICKED = "我必须等着.",
		},
		BIRDCAGE =
		{
			GENERIC = "鸟需要自由,但我需要它.",
			OCCUPIED = "别急!",--
			SLEEPING = "快醒醒!我需要你的蛋!",
		},
		BIRDTRAP = "加点小树枝和蛛丝就能做出来.",
		BIRD_EGG = "蛋蛋的忧伤.",
		BIRD_EGG_COOKED = "我喜欢煎鸡蛋.",
		BISHOP = "它在祈祷什么?",
		BLOWDART_SLEEP = "不会睡得很死, 只是短暂的休眠.",
		BLOWDART_PIPE = "远程武器.",
		BLOWDART_FIRE = "烧掉谁的屁屁呢?",
		BLUEGEM = "真漂亮",
		BLUEPRINT = "无论谁画的,它都已经死了.",
		BELL_BLUEPRINT = "无论谁画的,它都已经死了.",
		BLUE_CAP = "蘑菇好可爱.",
		BLUE_CAP_COOKED = "美味的烤蘑菇.",
		BLUE_MUSHROOM =
		{
			GENERIC = "陈旧的.",
			INGROUND = "如果可以的话, 我应该冷藏起来.",
			PICKED = "捡起来.",
		},
		BOARDS =
		{
			"木板是一个很好的燃料!",
			"我需要很多木板!",
		},
		BONESTEW = "好吃又管饱.",
		BUGNET = "抓小虫子咯.",
		BUSHHAT = "我是一个灌木丛.",
		BUTTER = "我是欧皇!",--
		BUTTERFLY =
		{
			GENERIC = "漂亮,但太短暂.",
			HELD = "它的命在我手里.",
		},
		BUTTERFLYWINGS = "没有翅膀蝴蝶就不能飞了.",
		BUTTERFLYMUFFIN = "甜甜的幸福感!",
		CAMPFIRE =
		{
			EMBERS = "黑夜要来了.",
			GENERIC = "我会活过今晚的!",
			HIGH = "像地狱!",
			LOW = "黑暗侵蚀.",
			NORMAL = "这是火.",
			OUT = "快没有燃料了.",
		},
		CANE = "这能让我跑得更快.",
		CARROT = "这是兔兔最喜欢的胡萝卜.",
		CARROT_COOKED = "烤过的胡箩卜真好吃.",
		CARROT_PLANTED = "这颗长在地上的胡萝卜可以采摘了.",
		CARROT_SEEDS = "总有一天, 这颗种子也会变成胡萝卜.",
		CAVE_FERN = "黑暗中的一点色彩.",
		CHARCOAL = "被地狱之火烧过了.",
        CHESSJUNK1 = "没有感情的机械生物.",
        CHESSJUNK2 = "没有感情的机械生物.",
        CHESSJUNK3 = "没有感情的机械生物.",
		CHESTER = "啊呜, 这是一只可爱的小狗狗!",--
		CHESTER_EYEBONE =
		{
			GENERIC = "它在盯着我.",
			WAITING = "没有那么多的睡眠... 等待.",
		},
		COOKEDMANDRAKE = "这是一份好食物. 但最好别直接吃.",
		COOKEDMEAT = "总是牛排当晚餐吗!",--
		COOKEDMONSTERMEAT = "煮熟的怪物肉,看上去比我还邪恶.",
		COOKEDSMALLMEAT = "开吃了!",--
		CORN = "这些乌鸦可能会为了它挖我的坟墓.",--
		CORN_COOKED = "还记得小时候街头卖爆米花的老爷爷吗!?",
		CORN_SEEDS = "这是一颗种子.",
		CROW =
		{
			GENERIC = "天下乌鸦一般黑!",
			HELD = "这是我的.",
		},
		CUTGRASS = "我喜欢猎杀一些小东西.",
		CUTREEDS = "我喜欢收割它.",
		CUTSTONE = "它能让我能修建好多建筑.",
		DEADLYFEAST = "这是最后的晚餐.",
		DEERCLOPS = "我能打得过它!",
		DEERCLOPS_EYEBALL = "Kindred别怕,它已经死了.",
		DEPLETED_GRASS =
		{
			GENERIC = "一丛草.",
		},
		DIRTPILE = "哦 看. 好多脏东西.",
		DIVININGROD =
		{
			COLD = "源头很遥远.",
			GENERIC = "它永远在寻找它失去的另一半.",
			HOT = "一些邪恶的东西在这里!",
			WARM = "邪恶接近了.",
			WARMER = "已经非常接近了.",
		},
		DIVININGRODBASE =
		{
			GENERIC = "这是什么新玩意儿嘛?",
			READY = "它仍然需要一个大的钥匙.",
			UNLOCKED = "让我们看看我们能找到什么!",
		},
		DRAGONFLY =
		{
			"也许我吃饱后能打得过他?",
			"它看起来很生气!",
		},
		LAVASPIT = 
		{
			HOT = "它吐了.",
			COOL = "冷却下来的气味真有趣.",
		},
		DRAGONFRUIT = "这根本不像龙的外形.",
		DRAGONFRUIT_COOKED = "还不是龙. 至少它已经死了.",
		DRAGONFRUIT_SEEDS = "它的种子. 它不会长出一条龙.",
		DRAGONPIE = "我自己做的! 想给Kindred尝尝.",
		DRUMSTICK = "做一份有营养的食品!",
		DRUMSTICK_COOKED = "熟一点的就更美味一点.",
		DUG_BERRYBUSH = "我想它死了, 我应该种了它.",
		DUG_GRASS = "我想它死了, 我应该种了它.",
		DUG_MARSH_BUSH = "我想它死了, 我应该种了它.",
		DUG_SAPLING = "我想它死了, 我应该种了它.",
		DURIAN = "带刺而且很臭!",
		DURIAN_COOKED = "没有刺了, 但是还是很臭!",
		DURIAN_SEEDS = "奇怪, 种子不臭.",
		EARMUFFSHAT = "我戴上它可爱吗?",
		EGGPLANT = "致命的茄子.",
		EGGPLANT_COOKED = "烹饪使它更美味.",
		EGGPLANT_SEEDS = "这是茄子的种子.",
		ACORN = 
		{
		    GENERIC = "一个小生命, 脆弱很容易逝去.",
		    PLANTED = "小树~小树~快快长大!!!.",
		},
		EVERGREEN =
		{
			BURNING = "这棵树燃烧着.",
			BURNT = "点着了.",
			CHOPPED = "一切都死了.",
			GENERIC = "这是颗松树.你猜它上面会有松鼠吗?",
		},
		EVERGREEN_SPARSE =
		{
			BURNING = "这棵树燃烧着.",
			BURNT = "点着了.",
			CHOPPED = "一切都死了.",
			GENERIC = "它很稀有，真想看看它变成树人的样子.",
		},
		EYEPLANT = "他们跟着主人形影不离.",
		FEATHERHAT = "时髦的帽子",--
		FEATHER_CROW = "乌鸦的羽毛, 黑色的像泥鳅.",
		FEATHER_ROBIN = "红色的羽毛, 红得像火焰.",
		FEATHER_ROBIN_WINTER = "雪鸟的羽毛, 白得像云彩.",
		FEM_PUPPET = "我好奇它做了什么.",
		FIREFLIES =
		{
			GENERIC = "这荧光真美,好想跟Kindred一起看.",
			HELD = "小小的灯光, 搁浅在我的口袋外面的世界.",
		},
		FIREHOUND = "狗狗着火了!",--
		FIRESTAFF = "小心,我不想再看见大火了.",--
		FISH = "这是在为生命拼命扑腾. 可怜的小鱼.",
		FISHINGROD = "这个, 我可以把小鱼们放到我的鱼缸里.",
		FISHSTICKS = "完美成型的鱼块,但别给我吃.",
		FISHTACOS = "加了玉米的我也不要.",
		FISH_COOKED = "这是做给Kindred的.",
		FLINT = "这是种锋利的石头.",
		FLOWER = "想送给Kindred.",
		FLOWERHAT = "Kindred戴着会很好看...",
		FLOWER_EVIL = "只有恶魔才会觉得它们好看吧.",
		FOLIAGE = "我记得风滚草里会有的.",
		FOOTBALLHAT = "Kindred给我做的. 好看吗?",
		FROG =
		{
			DEAD = "蛙生是如此的短暂和渺小.",
			GENERIC = "它很小.",
			SLEEPING = "它睡着了.",
		},
		FROGGLEBUNWICH = "我从来没想过青蛙腿会这么好.",
		FROGLEGS = "嗯. 青蛙没有腿还会跳吗?",--
		FROGLEGS_COOKED = "绝对美味.",
		FRUITMEDLEY = "甜, 一杯美味.",
		GEARS = "机械的重要零件.",
		GHOST = "它从坟墓里爬出来了!",
		GOLDENAXE = "我可以砍更多树.",
		GOLDENPICKAXE = "我可以凿出更多岩石.",
		GOLDENPITCHFORK = "至少我可以看到地球的风格.",
		GOLDENSHOVEL = "我可以挖的更多.",
		GOLDNUGGET = "发财了.",
		GRASS =
		{
			BARREN = "我需要肥料浇灌它.",
			WITHERED = "它只是不能承受太热的环境.",
			BURNING = "它看起来很有趣.",
			GENERIC = "它只是一株草.",
			PICKED = "小草,小草,快快长大!",
		},
		GREEN_CAP = "千万不能生吃.",
		GREEN_CAP_COOKED = "好好吃呦.",
		GREEN_MUSHROOM =
		{
			GENERIC = "小精灵的蘑菇房.",
			INGROUND = "如果可以的话, 我要藏起来.",
			PICKED = "收获.",
		},
		GUNPOWDER = "嘭!",
		HAMBAT = "感觉如何?被一块肉攻击! 哈哈哈!",--
		HAMMER = "哦,我可能会粉碎世界.",
		HEALINGSALVE = "有这个我就没事了Kindred.",
		HEATROCK =
		{
			FROZEN = "冰冷的像我的心.",
			COLD = "只是一个冰冷的岩石块.",
			GENERIC = "它能帮我顺利的度过夏天和冬天.",
			WARM = "暖暖的像妈妈的怀抱,真好.",
			HOT = "热得发亮, 热得越快.",
		},
		HONEY = "和Kindred一样,甜到心里了.",
		HONEYCOMB = "一片蜂巢.",
		HONEYHAM = "它会产蜜.",
		HONEYNUGGETS = "它会产蜜.",
		HORN = "听起来有牛被困在里面了.",
		HOUND =
		{
			"邪恶... 可恶... 野蛮!",
			"坏狗狗是要被惩罚的, 对吧?",
			"坏狗狗!",
		},
		HOUNDBONE = "有些是无法避免的. 死亡就是其中之一.",--
		HOUNDMOUND = "这里最恐怖的生物.",
		ICEBOX = "食物, 食物, 我的食物在哪里?",--
		ICEHOUND = "你要和我打雪仗吗?小狗狗.",
		INSANITYROCK =
		{
			ACTIVE = "这里写我的名字了吗.",
			INACTIVE = "黑暗和冰冷.",
		},
		JAMMYPRESERVES = "完美的浆果.",
		KABOBS = "肉串.",
		KILLERBEE =
		{
			GENERIC = "危险!别靠近.",
			HELD = "小家伙,你跑不掉了.",
		},
		KNIGHT = "等等…骑士?",
		KOALEFANT_SUMMER = "它预示着夏天要来了吗.",
		KOALEFANT_WINTER = "冬天会有它的身影.",
		KRAMPUS = "圣诞! 要来了吗!? 我要跟Kindred一起过圣诞!",--
		KRAMPUS_SACK = "它能给我装下好多东西.",
		LEIF = "这是树人雷夫.",
		LEIF_SPARSE = "树神雷夫,好威武.",
		LIGHTNING_ROD =
		{
			CHARGED = "嗞啦嗞啦,嗞嗞.",
			GENERIC = "有了这个再也不担心会被雷劈了.",
		},
		LITTLE_WALRUS = "带我去找你爸爸.",
		LIVINGLOG = "它永远在无声的尖叫.",
		LOCKEDWES = "麦斯威尔的雕像, 永远的活在饥荒的世界.",
		LOG = "如果树能说话, 它会说请珍惜好我的小心肝儿.",
		LUREPLANT = "它吞噬所有.",
		LUREPLANTBULB = "生命的延续.",
		MALE_PUPPET = "我好奇它做了什么.",
		MANDRAKE =
		{
			DEAD = "唉,可怜的曼德拉!",
			GENERIC = "真可爱. 我应该晚上带它回家.",
			PICKED = "前进, 我的曼德拉士兵!",
		},
		MANDRAKESOUP = "煮了它.",
		MARBLE = "光滑而坚硬.",
		MARBLEPILLAR = "即使石头也不能承受磨损.",
		MARBLETREE = "这棵树在跟寒冷抗争.",
		MARSH_BUSH =
		{
			BURNING = "就像荆棘一样燃烧.",
			GENERIC = "好多刺呦,我得小心点.",
			PICKED = "没事的Kindred, 小伤.",
		},
		MARSH_PLANT = "这是一种植物.",
		MARSH_TREE =
		{
			BURNING = "它在燃烧",
			BURNT = "它让我想起了乌鸦的羽毛",
			CHOPPED = "它的尖峰没有保护.",
			GENERIC = "一棵树,只知道痛苦.",
		},
		MEAT = "它是生的.",
		MEATBALLS = "看着让我流口水.",
		MEAT_DRIED = "肉晒干了. 更美味了.",
		MERM = "可怕的沼泽!",
		MINERHAT = "免提照明!",
		MONSTERMEAT = "恶魔也不会愿意吃的.",
		MONSTERMEAT_DRIED = "晒干了也不想吃.",
		MOSQUITO =
		{
			GENERIC = "离我远点!",
			HELD = "受死吧!害虫!",
		},
		MOSQUITOSACK = "一代蚊子血.",
		MOUND =
		{
			DUG = "我好像干了件缺德事.",
			GENERIC = "安息吧.",
		},
		NIGHTLIGHT = "邪恶而美丽.",
		NIGHTMAREFUEL = "可以代替灵魂的新食物!",
		NIGHTSWORD = "不如我的太刀.",
		NITRE = "硝石能带我度过炎热的夏季.",
		ONEMANBAND = "跟我跳支舞吧!",--
		PANDORASCHEST = "如果打开盒子会发生什么?",--
		PANFLUTE = "音乐填满我空虚的心灵.",
		PAPYRUS = "只是一张纸.",
		PENGUIN = "你们来自马达加斯加吗?",
		PERD = "远离我的浆果!",
		PEROGIES = "美味的水饺.",
		PETALS = "好像不是我想的那样.",
		PETALS_EVIL = "他们把我的灵魂涂上了颜色.",
		PICKAXE = "这样我们可以凿穿地球.",
		PIGGYBACK = "它死了更有用.",
		PIGKING = "把金子交出来.",
		PIGMAN =
		{
			DEAD = "它现在好了.",
			FOLLOWER = "我仍然感到孤独.",
			GENERIC = "小猪猪,笨笨哒.",
			GUARD = "难不倒我.",
			WEREPIG = "它不再友好了!",
		},
		PIGSKIN = "猪屁股?",
		PIGTORCH = "它在消减黑暗.",
		PINECONE = 
		{
		    GENERIC = "一个脆弱的小生命很容易逝去.",
		    PLANTED = "小树, 小树, 快快长大.",
		},
		PITCHFORK = "什么邪恶的工具!",
		PLANTMEAT = "食人花的肉.",
		PLANTMEAT_COOKED = "烤熟了更好吃.",
		PLANT_NORMAL =
		{
			GENERIC = "一种可食用的植物.",
			GROWING = "继续成长. 我会等你.",
			READY = "我已经准备好了.",
			WITHERED = "失去了生命的活力.",
		},
		POMEGRANATE = "好多石榴籽,像珍珠一样美丽!",
		POMEGRANATE_COOKED = "熟透了.",
		POMEGRANATE_SEEDS = "我希望有更多的种子.",
		POND = "小鱼鱼? 你在里面吗?",
		POOP = "咦惹, 好臭好恶心.",
		PUMPKIN = "又能吃又能用来过万圣节.",
		PUMPKINCOOKIE = "Kindred也是我的小饼干!",
		PUMPKIN_COOKED = "我喜欢.",
		PUMPKIN_LANTERN = "万圣节到了! ",
		PUMPKIN_SEEDS = "它只是种子.",
		PURPLEGEM = "紫色的就像... 薰衣草那样美丽.",
		RABBIT =
		{
			GENERIC = "小兔兔,让我摸摸你!",--
			HELD = "它现在安全了.",
		},
		RABBITHOLE = 
		{
			GENERIC = "我会掉下去变成爱丽丝吗?",
			SPRING = "兔兔去度假了.",--
		},
		RATATOUILLE = "一堆煮熟的蔬菜.",
		RAZOR = "我还没长胡子, 但其他动物就不一定了.",
		REDGEM = "红得像小苹果.",
		RED_CAP = "红蘑菇,不能生吃哦.",
		RED_CAP_COOKED = "可以把它和别的煮在一起.",
		RED_MUSHROOM =
		{
			GENERIC = "蘑菇长大了.",
			INGROUND = "如果可以的话, 我会藏起来.",
			PICKED = "收获.",
		},
		REEDS =
		{
			BURNING = "很快成为灰烬.",
			GENERIC = "这只是一堆芦苇.",
			PICKED = "收获.",
		},
        RELIC = 
        {
            GENERIC = "一个古老世界的残骸.",
            BROKEN = "破碎的石头",
        },
        RUINS_RUBBLE = "一堆破碎的梦.",
		RESURRECTIONSTONE = "一都是徒劳.",
		ROBIN =
		{
			GENERIC = "这意味着春天即将到来吗? 我希望不是.",
			HELD = "它喜欢被困在我的口袋里.",
		},
		ROBIN_WINTER =
		{
			GENERIC = "这意味着冬天即将到来吗? 我希望不是.",
			HELD = "它喜欢被困在我的口袋里.",
		},
		ROBOT_PUPPET = "我好奇它在想什么.",
		ROCK_LIGHT =
		{
			GENERIC = "熔岩坑.",
			OUT = "它看起来是无害的.",
			LOW = "阴影的侵蚀.",
			NORMAL = "熔岩的火.",
		},
		ROCK = "不是永久性的.",
		ROCKS = "一些小石头.",
        ROOK = "乌鸦? 还是一座城堡?",
		ROPE = "一捆绳子.",
		ROTTENEGG = "不能吃了, 但还有其他用处.",
		SANITYROCK =
		{
			ACTIVE = "这是给我的.",
			INACTIVE = "它离开了我.",
		},
		SAPLING =
		{
			BURNING = "燃起熊熊大火.",
			WITHERED = "它枯萎后得到了这个.",
			GENERIC = "它在慢慢长大.",
			PICKED = "树苗 树苗 快快长大.",
		},
		SEEDS = "生命, 它的承诺.",
		SEEDS_COOKED = "烤瓜子.",
		SEWING_KIT = "坏了, 我忘记怎么缝衣服了.",
		SHOVEL = "什么可怕的秘密要被我发现了?",
		SILK = "很黏.",
		SKELETON = "在他身上发生了惨剧.",
		SKELETON_PLAYER = "我是如此的接近它.",
		SMALLBIRD =
		{
			GENERIC = "你好, 小家伙.",
			HUNGRY = "你饿了吗?",
			STARVING = "你要饿晕了吗.",
		},
		SMALLMEAT = "小肉肉.",
		SMALLMEAT_DRIED = "肉晒干了更美味了.",
		SPEAR = "战斗! ",
		SPIDER =
		{
			DEAD = "我们会再见的.",
			GENERIC = "这是我的肉丸子!",--
			SLEEPING = "它只是睡着了, 永远.",
		},
		SPIDERDEN = "一窝团结的小动物.",
		SPIDEREGGSACK = "一个小小的卵里面有好多可爱的小蜘蛛.",
		SPIDERGLAND = "即使死了,还是很痛.",
		SPIDERHAT = "带上它我就是蜘蛛的家人了.",
		SPIDERQUEEN = "它是蜘蛛的女王.",
		SPIDER_WARRIOR =
		{
			DEAD = "唉, 勇敢的战士.",
			GENERIC = "一位骑士陨落了!",
			SLEEPING = "我必须小心.",
		},
		SPOILED_FOOD = "不! 我的食物! ",
		STATUEHARP = "也许这些雕塑将一直陪伴着我.",
		STATUEMAXWELL = "他把我带到了这里.",
		STINGER = "锋利致命.",
		STRAWHAT = "比不上我的斗笠.",
		STUFFEDEGGPLANT = "热腾腾的茄子.",
		SUNKBOAT = "一个残酷的玩笑!",
		SWEATERVEST = "穿上它我更像个老头了.",
		TAFFY = "你相信太妃糖可以让你的牙齿长蛀牙吗? 我觉得不会的.",--
		TALLBIRD = "Doppio一定很羡慕它.",--
		TALLBIRDEGG = "充满生命力.",
		TALLBIRDEGG_COOKED = "很残酷, 但很香.",
		TALLBIRDEGG_CRACKED =
		{
			COLD = "它感到冷了.",
			GENERIC = "一个微小脆弱的生命在努力降生.",
			HOT = "它快热成熟肉了.",
			LONG = "生命越早开始而生命就会越早结束.",
			SHORT = "很快.",
		},
		TALLBIRDNEST =
		{
			GENERIC = "蛋代表新生活.",
			PICKED = "鸟巢是空的.",
		},
		TEENBIRD =
		{
			GENERIC = "你会离开我吗?",
			HUNGRY = "我们都会饿的.",
			STARVING = "你要离开我了吗!",
		},
		TELEPORTATO_BASE =
		{
			ACTIVE = "我相信下一个世界会更糟糕!",
			GENERIC = "没有比这更好的了.",
			LOCKED = "它是锁着的.",
			PARTIAL = "难道是半建成, 或半毁?",
		},
		TELEPORTATO_BOX = "它是我到下一个世界重要的部件.",
		TELEPORTATO_CRANK = "它看起来很复杂.",
		TELEPORTATO_POTATO = "什么可怕的创作!",
		TELEPORTATO_RING = "它没有锋利的边缘.",
		TENTACLE = "长而薄,危险!",
		TENTACLESPOTS = "这些就不错.",
		TENTACLESPIKE = "锋利, 黏糊和危险.",
        TENTACLE_PILLAR = "所以这就是去下面了.",
        TENTACLE_PILLAR_ARM = "宝宝!",
        TENTACLE_GARDEN = "这是一个不同的.",
		TOPHAT = "我要把它戴在头上.",
		TORCH = "在深夜的一个小小的堡垒.",
		TRAP = "小蜘蛛和小兔兔最害怕这个.",
		TRAP_TEETH = "小狗狗最害怕这个.",
		TRAP_TEETH_MAXWELL = "这是麦斯威尔制作的陷阱.",
		TRINKET_1 = "好难看.",
		TRINKET_10 = "谁丢了他的假牙.",
		TRINKET_11 = "幸运的机器人, 它没有灵魂.",
		TRINKET_12 = "不再那么粘粘糊糊,是吗?",
		TRINKET_2 = "这是假的. 用来诱惑你犯罪.",
		TRINKET_3 = "如果刀能解决我的问题...",
		TRINKET_4 = "我会把这带到世界的尽头.",
		TRINKET_5 = "我将追逐着阳光奔跑.",
		TRINKET_6 = "一旦充满电它就会结束了.",
		TRINKET_7 = "只是一个简单的游戏.",
		TRINKET_8 = "我希望我能给它洗给澡.",
		TRINKET_9 = "生活的按钮.",
		TRUNKVEST_SUMMER = "温暖覆盖在我的皮肤上.",
		TRUNKVEST_WINTER = "它温暖我的身体.",
		TRUNK_COOKED = "它看起来更糟了.",
		TRUNK_SUMMER = "我想要更好的耳朵.",
		TRUNK_WINTER = "脱离他的唯一朋友.",
		TURKEYDINNER = "Kindred来跟我一起吃. ",
		TWIGS = "它是一束小树枝.",
		UMBRELLA = "我讨厌雨!",
		WAFFLES = "淋上糖浆后味道绝佳.",
		WALL_HAY_ITEM = "他们的防御值值得怀疑.",
		WALL_STONE = "那些乌合之众再也进不来了.",
		WALL_STONE_ITEM = "用石头制作的结实的墙.",
		WALL_RUINS = "他们没有保护远古人吗!",
		WALL_RUINS_ITEM = "它们闹鬼?",
		WALL_WOOD_ITEM = "质量一般的防御.",
		WALRUS = "把你的牙给我!",
		WALRUSHAT = "我戴这个好看吗? ",
		WALRUS_CAMP =
		{
			EMPTY = "如果他们不在那里, 那他们在哪里?",
			GENERIC = "这个房子让我感到很害怕.",
		},
		WALRUS_TUSK = "终于啊!",
		WASPHIVE = "它们躲在地下的堡垒.",
		WETGOOP = "操!",
		WINTERHAT = "它有助于保暖.",
		WORMHOLE =
		{
			GENERIC = "看这个! 巨大的嘴!",
			OPEN = "这把我带到哪里去?",
		},
		ACCOMPLISHMENT_SHRINE = "我从不关心隐喻.",
	},
	DESCRIBE_GENERIC = "这是某种……东西.",
	DESCRIBE_TOODARK = " 好黑! ",
	DESCRIBE_SMOLDERING = "这是被吞噬的火焰.",
	EAT_FOOD =
	{
		TALLBIRDEGG_CRACKED = "我这是要当爸爸还是妈妈了? ",
	},

}
